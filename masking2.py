...
def create_object_mask(image_path, output_path):
    ...
    binary_mask_path = os.path.join(output_path, "binary_mask.jpg")
    ...
    edges_image = cv2.cvtColor(binary_mask, cv2.COLOR_GRAY2BGR).copy()
    cv2.drawContours(edges_image, contours, -1, (0, 255, 0), 2)
    edges_image_path = os.path.join(output_path, "edges_image.jpg")
    cv2.imwrite(edges_image_path, edges_image)
